...
public class Zip {
    ...
    public class func unzipFile(_ zipFilePath: URL, destination: URL, overwrite: Bool, password: String?, progress: ((progress: Double) -> ())?) throws {
        ...
        repeat {
            ...
            let url = try! destination.appendingPathComponent(pathString)
            
            guard let fullPath = url.path else {
                throw ZipError.unzipFail
            }
            ...
            if !isDirectory {
                ...
                do {
                    ...
                    if let fileDate = fileAttributes[FileAttributeKey.modificationDate] as? Date {
                        ...
                        zipInfo.tmz_date.tm_sec = UInt32(components.second ?? 0)
                        zipInfo.tmz_date.tm_min = UInt32(components.minute ?? 0)
                        zipInfo.tmz_date.tm_hour = UInt32(components.hour ?? 0)
                        zipInfo.tmz_date.tm_mday = UInt32(components.day ?? 0)
                        zipInfo.tmz_date.tm_mon = UInt32(components.month ?? 1) - 1
                        zipInfo.tmz_date.tm_year = UInt32(components.year ?? 0)
                    }
